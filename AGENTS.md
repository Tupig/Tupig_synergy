...
Applies to every change in this repository. When a rule here conflicts with an ad-hoc request, raise the
...
适用于本仓库的所有改动。若本文件的规则与临时要求冲突，应当**先提出冲突**，而不是默默破坏约定。
...
- `vendor/vcpkg` 是浅克隆；CMake 配置前必须确保固定 baseline 提交存在。
